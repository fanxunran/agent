...
const export2Excel = (theadData, tbodyData, dataname) => {
  ...
  for (let i = 0; i < tb_len; i++) {
    ...
    theadData.forEach(item => {
      if (re.test(row[item.key])) { // 如果为图片，则需要加div包住图片
        //
        tbody += '<td style="width:' + width + 'px; height:' + height + 'px; text-align: center; vertical-align: middle"><div style="display:inline"><img src=\'' + row[item.key] + '\' ' + ' ' + 'width=' + '\"' + width + '\"' + ' ' + 'height=' + '\"' + height + '\"' + '></div></td>';
      } else {
        tbody += '<td style="text-align:center">' + row[item.key] + '</td>';
      }
    });
    tbody += '</tr>';
  }
